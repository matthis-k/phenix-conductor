...
- [ ] Rework credential secret storage around `secrecy` and standard credential/keyring facilities where platform support is acceptable; document the fallback policy. Active work outside this sub-sweep: `phenix-backend-native::credentials` still stores `StoredCredential::ApiKey`/`OAuth` secrets and `phenix-domain::AuthenticationInput::ApiKey` as raw `String`. `phenix-provider-sdk` already models `Secret`/`Token` newtypes with `<redacted>` `Debug` and a boundary `OAuthCredential` store, so the canonical redaction surface exists; aligning `backend-native` persistence and the domain `AuthenticationInput` against it (and choosing the keyring fallback policy) is a coordinated persistence/domain change tracked here rather than a mechanical adoption in this PR's main sweep.
