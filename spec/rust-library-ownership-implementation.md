...
- [ ] Rework credential secret storage around `secrecy` and standard credential/keyring facilities where platform support is acceptable; document the fallback policy. Partially advanced by the OAuth migration: `oauth2`'s `AccessToken`/`RefreshToken` wrappers (used in the exchange/refresh paths) are secret-backed in the crate, and `ClientId` is treated as public configuration and kept non-secret. The remaining step is a coordinated persistence/domain change, not a mechanical swap: `phenix-backend-native::StoredCredential::{ApiKey, OAuth}` and `phenix-domain::AuthenticationInput::ApiKey` are `serde`-serialized (the credential-file JSON and the client auth wire). `secrecy` 0.10 only derives `Serialize` on the newer `SecretBox<T: SerializableSecret>` and deliberately omits it on `Secret<T>` (see the crate docs), so folding these fields into `Secret`/`SecretBox` requires migrating the persisted credential-file shape and the public `AuthenticationInput` wire type in tandem, plus choosing the keyring fallback policy. Left active for a dedicated persistence/domain change.
