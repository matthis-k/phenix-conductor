...
- [ ] Replace hand-written OAuth PKCE/token/refresh mechanics with `oauth2` and maintained HTTP/JWT helpers; retain Codex provider policy and UX. Planned adoption: add `oauth2` (configured `default-features = false`) with a thin `AsyncHttpClient` adapter over the existing `reqwest 0.13`, so the PKCE challenge/verifier, authorize URL, `authorization_code` exchange, and `refresh_token` exchange become `oauth2`-owned. Add `jsonwebtoken` for JWT handling, distinguishing unverified claim extraction (`dangerous::insecure_decode`) from signature/claims verification where the Codex id-token is genuinely validated; merely reading `exp` does not validate expiry. Keep repo-owned: the Codex provider config (`CLIENT_ID`, issuer/token/responses endpoints, scope, `codex_*`/`originator`/`version` extra params), the local callback ports/timeouts/response page, and the `StoredCredential` persistence model. `oauth2`'s typed `AccessToken`/`RefreshToken`/`ClientId` are used; `ClientId` is public configuration so it is not a secret. The callback HTTP request-line parsing should only be hand-written if no existing first-party HTTP-server dependency is available to delegate it to (currently `hyper` is transitive via `reqwest` only).
- [ ] Rework credential secret storage around `secrecy` and standard credential/keyring facilities where platform support is acceptable; document the fallback policy. Planned with the OAuth adoption: `oauth2`'s `AccessToken`/`RefreshToken` wrappers are secret-backed in the crate but are not `secrecy::SecretString`; `ClientId` is public configuration and must remain non-secret. The `phenix-backend-native::StoredCredential::{ApiKey, OAuth}` secrets and `phenix-domain::AuthenticationInput::ApiKey` are still raw `String`; align them against `secrecy::Secret<String>` for storage/redaction and `phenix-provider-sdk`'s existing `Secret`/`Token` boundary types, and choose the keyring fallback policy. This is a coordinated persistence/domain change that accompanies the OAuth migration rather than an isolated mechanical one.
...
- [ ] Keep provider normalization/policy, but remove hand-maintained generic wire mechanics where pinned/generated upstream protocol types are practical. Provider SDK's `send_http` bridge, `apply_auth`, `normalize_http_error`, and `RateLimits::from_headers` remain the intentional Phenix-owned provider policy and normalization layer on top of canonical `http`/`bytes`/`url` types and the `reqwest` transport; they are not generic wire mechanics to outsource. The full MCP/OAuth protocol migrations that would replace the remaining wire state machines are tracked under their own items above.
