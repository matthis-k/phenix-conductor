...
9. acknowledge only after future model-tool projection can observe the tool.
...
The application host projects admitted client descriptors into the same backend-neutral model surface used by runtime/plugin tools:
...
Merge admitted client descriptors with the runtime/plugin `ModelToolDescriptor` set before model routing. Keep policy and implementation ownership in the host-side route metadata rather than leaking client origin to the provider.

Rules:

- no provider or backend learns whether the tool is client-hosted;
- duplicate effective ids are deterministic conflicts, not last-writer wins;
- model/provider preparation stays origin-neutral;
- model-visible schemas are the declared exact tool schemas;
- ordering is deterministic.

Legacy backend `ToolProvision` remains a separate backend abstraction where still used. Client admission does not need an additional backend-specific merge API when the active model path already consumes `ModelToolDescriptor`.
...
model emits ModelToolCall
  ...
  -> validate output
  -> ordinary ToolResult / ToolFailed
```
...
Never classify by error text. Ordinary `ToolFailed` remains structural when it carries the typed `ApplicationError` value.
...
- admitted client tool and runtime tool use the same `ModelToolDescriptor` shape at model routing;
- runtime/plugin and admitted-client descriptors merge before model invocation;
- duplicate effective ids fail before provider observation;
- permission-gated client tool asks through the normal permission path;
- no provider/backend client-tool special case is required.
...
```text
Lua client connects
-> registers fixture.client.echo with a Lua function
-> binding lifts handler into CallableRef
-> session admission exposes ordinary ModelToolDescriptor
-> host receives typed ModelToolCall
-> ordinary permission/tool lifecycle selects the admitted route
-> generic capability request reaches Lua host thread
-> typed output becomes ordinary ToolResult
-> stop removes admission
-> old callable/admission cannot execute
```

Reconnect generation/readmission semantics may be covered by the focused Rust regression when the packaged fixture does not restart the process.
