...
pub enum FrontendResponse {
    Providers { providers: Vec<LiveFrontendProvider> },
    Request { request: FrontendServiceRequest },
    Result { result: FrontendServiceResult },
    Updated,
}
...
impl PluginInstance for FrontendPlugin {
    ...
    fn invoke(
        ...
        let command: FrontendCommand = serde_json::from_slice(input).map_err(|error| error.to_string())?;
        let response = match command {
            FrontendCommand::SetProviders { connection_id, providers } => {
            ...
            FrontendCommand::Disconnect { connection_id } => {
                ...
                self.pending.retain(|_, call| call.connection_id != connection_id);
                FrontendResponse::Updated
            }
            FrontendCommand::Catalog => FrontendResponse::Providers {
                providers: self.catalog(),
            },
            FrontendCommand::BindRoot { execution_id, connection_id } => {
                ...
                if self.root_routes.insert(execution_id.clone(), connection_id).is_some() {
                    return Err(format!("root execution already has a frontend route: {execution_id}"));
                }
                FrontendResponse::Updated
            }
            ...
            FrontendCommand::BeginExecutionCall { execution_id, provider, method, params } => {
                let root = execution_root(host, &execution_id)?;
                let connection_id = self.root_routes.get(&root)
                    .cloned()
                    .ok_or_else(|| format!("execution has no live frontend root route: {execution_id}"))?;
                self.begin_call(connection_id, provider, method, params)?
            }
            FrontendCommand::BeginDirectCall { connection_id, provider, method, params } => {
                self.begin_call(connection_id, provider, method, params)?
            }
            FrontendCommand::CompleteCall { connection_id, correlation_id, result } => {
                ...
                FrontendResponse::Result {
                    result: FrontendServiceResult { correlation_id, result },
                }
            }
        };
...
impl FrontendPlugin {
    ...
    fn begin_call(
        ...
        self.next_correlation_id = self.next_correlation_id.checked_add(1)
            .ok_or_else(|| "frontend correlation id exhausted".to_owned())?;
        self.pending.insert(correlation_id, PendingCall { connection_id: connection_id.clone() });
...
fn execution_lookup(host: &PluginHost<'_>, execution_id: &str) -> Result<Option<crate::ExecutionRecord>, String> {
    let output = host.invoke_service(
        &execution_service(),
        &serde_json::to_vec(&ExecutionCommand::GetExecution { id: execution_id.to_owned() })
...
fn validate_provider(provider: &FrontendProviderDescriptor) -> Result<(), String> {
    ...
    if provider.capabilities.iter().any(|capability| capability.trim().is_empty()) {
...
mod tests {
    ...
    fn kernel() -> Kernel {
        ...
        let mut kernel = Kernel::new(KernelConfig::new([execution_manifest, frontend_manifest]).unwrap());
        kernel.register_embedded_factory(execution_id, execution_factory).unwrap();
        kernel.register_embedded_factory(frontend_id, frontend_factory).unwrap();
    ...
    fn descendant_calls_route_to_root_owner_and_wrong_connection_response_is_rejected() {
        ...
        invoke(&mut kernel, FrontendCommand::SetProviders {
            connection_id: "frontend-a".into(),
            providers: vec![FrontendProviderDescriptor { id: "web".into(), capabilities: BTreeSet::from(["search".into()]) }],
        }).unwrap();
        invoke(&mut kernel, FrontendCommand::BindRoot { execution_id: "root".into(), connection_id: "frontend-a".into() }).unwrap();
        let response = invoke(&mut kernel, FrontendCommand::BeginExecutionCall {
            execution_id: "child".into(), provider: "web".into(), method: "search".into(), params: serde_json::json!({"q":"nix"}),
        }).unwrap();
        ...
        assert!(invoke(&mut kernel, FrontendCommand::CompleteCall {
            connection_id: "frontend-b".into(), correlation_id, result: serde_json::json!({}),
        }).unwrap_err().contains("wrong connection"));
        assert!(matches!(
            invoke(&mut kernel, FrontendCommand::CompleteCall {
                connection_id: "frontend-a".into(), correlation_id, result: serde_json::json!({"ok":true}),
            }).unwrap(),
            FrontendResponse::Result { .. }
        ));
    }
    ...
    fn disconnect_removes_provider_catalog_routes_and_pending_calls_without_durable_restore() {
        ...
        invoke(&mut kernel, FrontendCommand::SetProviders {
            connection_id: "frontend-a".into(),
            providers: vec![FrontendProviderDescriptor { id: "web".into(), capabilities: BTreeSet::new() }],
        }).unwrap();
        invoke(&mut kernel, FrontendCommand::BindRoot { execution_id: "root".into(), connection_id: "frontend-a".into() }).unwrap();
        invoke(&mut kernel, FrontendCommand::Disconnect { connection_id: "frontend-a".into() }).unwrap();
        assert_eq!(
            invoke(&mut kernel, FrontendCommand::Catalog).unwrap(),
            FrontendResponse::Providers { providers: Vec::new() }
        );
        assert!(invoke(&mut kernel, FrontendCommand::BeginExecutionCall {
            execution_id: "root".into(), provider: "web".into(), method: "search".into(), params: serde_json::json!({}),
        }).unwrap_err().contains("no live frontend root route"));
    }
}
