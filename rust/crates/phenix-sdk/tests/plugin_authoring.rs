                ...
                |request: RunRequest| -> Result<RunResponse, String> {
                    ...
                    let response = if request.exact {
                        context
                            .sdk
                            .models
                            .invoke::<Exact<ModelResponse>>(&model_request)
                            .map(|response| response.0)
                    } else {
                        context.sdk.models.invoke::<ModelResponse>(&model_request)
                    }
...
#[test]
fn structural_matching_policy_is_selected_by_the_target_type() {
    let value = PhenixValue::from(&ProviderEvent {
        value: "ok".into(),
        extra: 7,
    });
    let expected = ProjectedEvent { value: "ok".into() };

    let projected = ProjectedEvent::try_from(&value).unwrap();
    assert_eq!(projected, expected);

    let projected = Project::<ProjectedEvent>::try_from(&value).unwrap();
    assert_eq!(projected.0, expected);

    let exact = Exact::<ExactEvent>::try_from(&value);
    assert!(exact.is_err());
}
